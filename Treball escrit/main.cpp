...
double x_bounds[2] = { -2, 1 };
double y_bounds[2] = { -1, 1 };
...
double XMultiplication(double x, double y, double cr) {
	x = x * x - y * y + cr;
	///x = x * x * x * x + y * y * y * y - 6 * x * x * y * y + cr;
	return x;
}

double YMultiplication(double x, double y, double ci) {
	y = 2 * y * x + ci;
	///y = 4 * x * x * x * y - 4 * x * y * y * y + ci;
	return y;
}
...
double iterate(int n, double cr, double ci, float threshold) {
	...
	for (int k = 0; k < n; k++)
	{
		...
		x = XMultiplication(x, y, cr);
		y = YMultiplication(y, x_t, ci);
	...
	{				
		...
		if (drawIterations && !update) {
			if (iter <= iterations) {
				...
				derivadaX = 2 * currentX;
				derivadaY = 2 * currentY;
