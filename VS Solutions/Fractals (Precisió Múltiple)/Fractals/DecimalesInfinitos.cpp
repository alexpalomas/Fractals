...
private:
	void SetAsDouble(double a) {
		entero = floor(absolute(a));
		signo = a >= 0;

		string s = to_string(absolute(a));
...
private:
	bool IgualQueSinSigno(Num a, Num b) {
		a.BorrarCeros();
		b.BorrarCeros();	
		
		bool returnable = true;
		returnable = a.entero == b.entero;
		if(returnable){
			returnable = a.decimales == b.decimales;
		}
		return returnable;
	}
...
public:	
	...
	bool IgualQue(Num a, Num b) {
		return IgualQueSinSigno(a, b) == (a.signo == b.signo);
		...
		*/
		...
		else
		{
			...
			if (MasGrandeQueSinSigno(a, b)) {
				...
				resultat.entero = (int)absolute((double)(a.entero - (b.entero + residu)));
				resultat.signo = a.signo;
			}
			else {
				// b es més gran que a
				...
				resultat.entero = (int)absolute((double)(b.entero - (a.entero + residu)));
		...
		// Si volem que trunqui, trunquem, si no, no.
		if(truncar)
			resultat.Truncar();
	...
	Num Restar(Num a, Num b) {
		return Sumar(a, InvertirSigno(b));
	}
	...
	Num Inverso(Num n) {
		...
		//result = 1 / n;
		/*
			Farem servir un métode de prova i error.
			Sabent que n es el nombre que ens donen i c el nombre que volem. 
			Tenim que 1 / n = c;
			...
			Llavors:
			Si n > 1:
				c ha de començar per 0, i anar fent.
			Si n < 1:
				c ha de començar per l'1 i cap amunt.
			Si n = 1:
				c = 1 també.
				
			Com fem el métde de prova i error?
			Exemple:
				1 / 4 = c;
				
				c * 4 = 1;
				
			Com sabem que 4 es més gran que 1, comencem pel 0,:
			Llavors tenim que:
				
				0,1 * 4 = 0,4; Seguim;
				0,2 * 4 = 0,8; Seguim;
				0,3 * 4 = 1,2; Més gran que 1, canviem de decimal;
				0,21 * 4 = 0,84; Seguim;
				0,22 * 4 = 0,88; Seguim;
				0,23 * 4 = 0,92; Seguim;
				0,24 * 4 = 0,96; Seguim;
			...
			I ja el tenim. Fácil, no?
		*/
		
		if(MasGrandeQueSinSigno(n, Num(1))) {
			result.entero = 0;
			result.decimales.insert(result.decimales.end(), 1);
			
			for(int i = 0; i < truncament; i++) {
				
				while(!MasGrandeQue(Abs(Multiplicar(result, n)), Num(1))) {
						result.decimales.back() += 1;
				}
				result.decimales.back() -= 1;
				
				if(IgualQue(Abs(Multiplicar(result, n)), Num(1))) {
					result.Truncar();
					return result;
				}
				result.decimales.insert(result.decimales.end(), 1);
...
int main(void) {
	...
	cout << endl << a_s << " + " << b_s << " = ";
	o.Sumar(a, b).WriteNumber();
	
	cout << endl << a_s << " - " << b_s << " = ";
	o.Restar(a, b).WriteNumber();
	
	cout << endl << a_s << " * " << b_s << " = ";
	o.Multiplicar(a, b).WriteNumber();
	
	cout << endl << a_s << " / " << b_s << " = ";
	o.Dividir(a, b).WriteNumber();
