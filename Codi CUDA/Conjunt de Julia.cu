...
// ITERACIONS AQUI //
__global__
void FractalCalculator(int dimensions, float *fractal, int iter, double x_bounds_s, double x_bounds_e, double y_bounds_s, double y_bounds_e, double cr, double ci) 
{
	...
	for (int i=index; i < dimensions; i+=stride) {
		...
		double x = x_bounds_s + normalizedX * espacioX;
		double y = y_bounds_e - normalizedY * espacioY;
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (animate) {
			...
			//c.theta += 0.01;
			c.theta += fElapsedTime / 2;
			//totalTime += fElapsedTime;

			//c.theta += PI / (float)180;
			//totalTime += PI / (float)180;
...
private:
	...
	void UpdateFractal(bool stop) {		
		//auto begin = chrono::high_resolution_clock::now();
		
		x_sum = (absolute(x_bounds[0] - x_bounds[1])) / width;
		y_sum = (absolute(y_bounds[0] - y_bounds[1])) / height;
		
		FractalCalculator<<<1, 1024>>>(dimensions, fractal, iterations, x_bounds[0], x_bounds[1], y_bounds[0], y_bounds[1], c.x, c.y);
		...
		if(!stop) {
			...
			ofstream myfile;
			myfile.open ("stadistics - CUDA.txt", ios::app);
			myfile << " & " << elapsedTime << " \\\\ \\hline" << "\n";
			myfile.close();*/
		}
